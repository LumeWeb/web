/**
 * Declaration / import-resolution guard for the ROOT source-info feature.
 *
 * A non-React Video.js v10 consumer must be able to import
 * `siaSourceInfoFeature`, `selectSiaSourceInfo` and `SiaSourceInfoState` from
 * the package root with only the non-React video.js packages installed
 * (`@videojs/core/dom`, `@videojs/store`, `@videojs/media`) — never
 * `@videojs/react`. The React hook (`/react`) may require `@videojs/react`,
 ...
 * from the package root. `index.js` re-exports `./sia-source-info-feature.js`,
 * so both files must stay react-free.
 */
const ROOT_DTS_FILES = ['index.d.ts', 'sia-source-info-feature.d.ts'];

describe('root source-info declarations stay consumable without @videojs/react', () => {
  ...
  it.skipIf(IS_BROWSER)(
    ...
    () => {
      const { exists, path, text } = builtFile('sia-source-info-feature.d.ts');
      expect(exists, `missing ${path}; run \`pnpm build\` first`).toBe(true);
...
const state: SiaSourceInfoState | undefined = selectSiaSourceInfo(store.state);
const active: boolean = state?.sourceInfo.active ?? false;
const window = state?.sourceInfo;
const info: SourceInfo | undefined = window?.active ? window.info : undefined;
void active;
void info;
`;
      ...
      expect(
        ...
        'root declarations must not force a non-React consumer to resolve @videojs/react; ' +
          're-run `pnpm build` after source changes and check dist/esm/sia-source-info-feature.d.ts',
      ).toEqual([]);
    },
  );
});
