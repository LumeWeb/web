/**
 * A minimal React harness for `useSiaLoad`: mounts a real `@videojs/react`
 * `createPlayer` `<Player>` built with `siaLoadFeature`, attaches the same
 * fake media the non-React load spec uses, and records every load-acceptance
...
import { useSiaLoad } from '../../react/index.tsx';
import {
  siaLoadFeature,
  type SiaLoadState,
} from '../../sia-load-feature.ts';
import type { SiaLoadChangeDetail } from '../../sia-video-source.ts';
import { FakeSiaMedia } from './fake-sia-media.ts';

const { Player } = createPlayer({
  displayName: 'SiaLoadTestPlayer',
  features: [siaLoadFeature],
});

export interface LoadReactHarness {
  ...
  snapshots: (SiaLoadState | undefined)[];
...
export async function mountLoadReactHarness(): Promise<LoadReactHarness> {
  const container = document.createElement('div');
  document.body.appendChild(container);
  const root = createRoot(container);
  const snapshots: (SiaLoadState | undefined)[] = [];
