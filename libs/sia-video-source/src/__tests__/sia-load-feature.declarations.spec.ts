/**
 * Declaration / import-resolution guard for the ROOT load feature.
 *
 * A non-React Video.js v10 consumer must be able to import `siaLoadFeature`,
 * `selectSiaLoad` and `SiaLoadState` from the package root with only the
 * non-React video.js packages installed (`@videojs/core/dom`,
 ...
 * from the package root. `index.js` re-exports `./sia-load-feature.js`,
 * so both files must stay react-free.
 */
const ROOT_DTS_FILES = ['index.d.ts', 'sia-load-feature.d.ts'];

describe('root load declarations stay consumable without @videojs/react', () => {
  ...
  it.skipIf(IS_BROWSER)(
    ...
    () => {
      const { exists, path, text } = builtFile('sia-load-feature.d.ts');
      expect(exists, `missing ${path}; run \`pnpm build\` first`).toBe(true);
...
const state: SiaLoadState | undefined = selectSiaLoad(store.state);
const accepted: boolean = state?.accepted ?? false;
void accepted;
`;
      ...
      expect(
        ...
        'root declarations must not force a non-React consumer to resolve @videojs/react; ' +
          're-run `pnpm build` after source changes and check dist/esm/sia-load-feature.d.ts',
      ).toEqual([]);
    },
  );
});
