...
const getTouchPoint = (touch: React.Touch | Touch): Point => ({
...
export const useDrag = ({
  ...
  const callbacksRef = React.useRef({ onEnd, onMove, onStart });
  ...
  React.useEffect(() => {
    callbacksRef.current = { onEnd, onMove, onStart };
  ...
  const onDrag = React.useCallback((pointInWindow: Point) => {
    ...
    if (callbacksRef.current.onMove) {
      callbacksRef.current.onMove({ point, pointInWindow });
    }
  }, []);
  ...
  const onMouseDown = React.useCallback(
    (e: React.MouseEvent<HTMLElement, MouseEvent>) => {
      ...
      if (
        knobs?.length &&
        !knobs.find((knob) => knob.contains(e.target as Node))
      ) {
        return;
      }
