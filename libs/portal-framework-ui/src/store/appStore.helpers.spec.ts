...
describe("appStore helpers", () => {
  describe("addMenuItems", () => {
    it("should handle complex nested menu structures correctly", () => {
      ...
      const parent1 = helpers.findMenuItem(appStore.getState().menuItems, "parent1");
      const parent2 = helpers.findMenuItem(appStore.getState().menuItems, "parent2");
      ...
      expect(appStore.getState().menuItems.some(item => item.id === "root1")).toBe(true);
      // Grandchild should be under child1 immediately since we process items individually
      const initialChild1 = helpers.findMenuItem(appStore.getState().menuItems, "child1");
      expect(initialChild1?.children).toHaveLength(1);
      expect(initialChild1?.children?.[0].id).toBe("grandchild1");
      expect(appStore.getState().menuItems.some(item => item.id === "grandchild1")).toBe(false);
      ...
      const updatedChild1 = helpers.findMenuItem(appStore.getState().menuItems, "child1");
    ...
    it("should add multiple items as children to a parent when parentKey is provided", () => {
      ...
      const parentItem = helpers.findMenuItem(appStore.getState().menuItems, "parent");
    ...
    it("should fall back to root when parentId is not found", () => {
      ...
      appStore.getState().addMenuItems(itemsToAdd);
      expect(appStore.getState().menuItems.some(item => item.id === "child1")).toBe(true);
    });
  });

  const mockMenuItems: NavigationItem[] = [
    { id: "dashboard", label: "Dashboard", path: "/dashboard", children: [] },
    { id: "settings", label: "Settings", children: [
      { id: "profile", label: "Profile", path: "/settings/profile", children: [] },
      { id: "account", label: "Account", path: "/settings/account", children: [] },
    ]},
    { id: "plugins", label: "Plugins", children: [] },
  ];
  ...
  describe("addItemsToRoot", () => {
    it("should add new items to the root level", () => {
      ...
      expect(result[1]).toEqual({ id: "item2", label: "Item 2", children: [] });
      expect(result[2]).toEqual({ id: "item3", label: "Item 3", children: [] });
    });
  ...
  describe("addItemsToChildren", () => {
    it("should add new items to the children of a parent item", () => {
      const parent: NavigationItem = { id: "parent1", label: "Parent 1", children: [] };
      const newItems: NavigationItem[] = [
        { id: "child1", label: "Child 1" },
        { id: "child2", label: "Child 2" },
      ];
      ...
      expect(result.children![0]).toEqual({ id: "child1", label: "Child 1", children: [] });
      expect(result.children![1]).toEqual({ id: "child2", label: "Child 2", children: [] });
    });

    it("should not add duplicate items to the children of a parent item", () => {
      const parent: NavigationItem = { id: "parent1", label: "Parent 1", children: [{ id: "child1", label: "Child 1" }] };
      const newItems: NavigationItem[] = [
  ...
  describe("findMenuItem", () => {
    ...
    it("should find a menu item by ID in a nested structure", () => {
      const items: NavigationItem[] = [
        { id: "item1", label: "Item 1", children: [{ id: "item2", label: "Item 2" }] },
      ];
      const result = helpers.findMenuItem(items, "item2");
      expect(result).toEqual({ id: "item2", label: "Item 2" });
    });
  ...
  describe("findAndModifyMenuItem", () => {
    it("should modify a root level item", () => {
      const modifier = (item: NavigationItem) => ({ ...item, label: "Modified Dashboard" });
      const result = helpers.findAndModifyMenuItem([...mockMenuItems], "dashboard", modifier);
      expect(result).toHaveLength(3);
    ...
    it("should modify a nested item", () => {
      const modifier = (item: NavigationItem) => ({ ...item, label: "Modified Profile" });
      const result = helpers.findAndModifyMenuItem([...mockMenuItems], "profile", modifier);
      expect(result).toHaveLength(3);
      ...
      const modifiedChild = settingsItem?.children!.find(item => item.id === "profile");
      expect(modifiedChild?.label).toBe("Modified Profile");
      const otherChild = settingsItem?.children!.find(item => item.id === "account");
    ...
    it("should return the original array if no item is found", () => {
      const modifier = (item: NavigationItem) => ({ ...item, label: "Should Not Happen" });
      const originalItems = [...mockMenuItems];
      const result = helpers.findAndModifyMenuItem(originalItems, "non-existent", modifier);
     ...
     it("should return the original array if no modification occurs in children", () => {
      ...
      const originalItems = [...mockMenuItems];
      const result = helpers.findAndModifyMenuItem(originalItems, "non-existent-child", modifier);
     ...
     it("should return a new array if modification occurs in children", () => {
      const modifier = (item: NavigationItem) => ({ ...item, label: "Modified Profile" });
      const originalItems = [...mockMenuItems];
      const result = helpers.findAndModifyMenuItem(originalItems, "profile", modifier);
      expect(result).not.toBe(originalItems); // Should return a new array reference
      const settingsItem = result.find(item => item.id === "settings");
      const originalSettingsItem = originalItems.find(item => item.id === "settings");
  ...
  describe("removeItemFromMenu", () => {
    it("should remove a root level item", () => {
      const result = helpers.removeItemFromMenu([...mockMenuItems], "dashboard");
      expect(result).toHaveLength(2);
    ...
    it("should remove a nested item", () => {
      ...
      expect(settingsItem?.children!.find(item => item.id === "profile")).toBeUndefined();
      expect(settingsItem?.children!.find(item => item.id === "account")).toBeDefined(); // Other child remains
    });
    ...
    it("should return the original array if key is not found", () => {
     ...
     it("should return the original array if key is not found in children", () => {
      const originalItems = [...mockMenuItems];
      const result = helpers.removeItemFromMenu(originalItems, "non-existent-child");
      expect(result).toBe(originalItems); // Should return the exact same array reference
    });
