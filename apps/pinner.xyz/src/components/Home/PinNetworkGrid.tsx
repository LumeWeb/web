import { Fingerprint, Server, Upload, Loader2 } from "lucide-react";
import { useNodeGrid } from "./useNodeGrid";

export default function PinNetworkGrid() {
  const { nodes, loading: cidLoading, packetPaths, activeCount, totalCount } = useNodeGrid();
    ...
    <div className="relative w-full max-w-[480px] mx-auto select-none">
      ...
      <div className="flex justify-center mb-8">
        ...
        >
          ...
          <div>
            <p className="text-white text-sm font-medium font-mono">bafybei...gtd7e</p>
            <p className="text-home-text-muted text-xs">
              {activeCount} of {totalCount} hosts proving storage
            </p>
          </div>
        </div>
      </div>

      {/* Connection lines from nodes to CID (SVG overlay) */}
      <svg
      ...
      <div className="grid grid-cols-3 gap-3">
        {nodes.map((node) => (
          ...
          >
            ...
            >
              {node.phase === "recovering" ? "REPAIR" : node.phase === "online" ? "PROVING" : "OFFLINE"}
            </p>
      ...
      <div className="flex justify-center mt-6">
        <div className="flex items-center gap-3 rounded-xl border border-home-text/10 bg-home-card-bg/50 px-5 py-3">
          <Upload className="h-5 w-5 text-home-accent" />
          <div>
            <p className="text-white text-sm font-medium">You</p>
            <p className="text-home-text-muted text-xs">Upload or pin</p>
          </div>
        </div>
      </div>

      {/* Replication status bar */}
