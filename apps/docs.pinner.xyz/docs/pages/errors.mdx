...
Every API error response includes a `code` field for programmatic handling. This page lists every code the API can return, when it happens, what to do about it, and whether it's safe to retry.
...
|------|------|----------------|------------|--------|
| `UNAUTHORIZED` | 401 | API key is invalid, expired, or revoked | Run `pinner auth` to re-authenticate. Check `PINNER_API_KEY` env var. | No |
...
| Code | HTTP | When it happens | What to do | Retry? |
|------|------|----------------|------------|--------|
| `ErrUnsupportedFormat` | 422 | File format isn't supported | Use a supported format (ZIP, CAR, or raw files). | No |
| `ErrCorruptedFile` | 500 | File is corrupted | Re-export or re-download the file and retry. | No |
| `ErrEmptyZIP` | 500 | ZIP archive has no files | Add content to the ZIP and retry. | No |
| `ErrPasswordProtected` | 500 | File is password-protected | Remove password protection and retry. | No |
| `ErrFileUploadFailed` | 500 | Upload failed mid-transfer | Check network and retry with backoff. | Yes |
...
| Code | HTTP | When it happens | What to do | Retry? |
|------|------|----------------|------------|--------|
| `WEBSITE_NOT_FOUND` | 404 | Website doesn't exist | Check the website ID. See [Websites](/websites/concepts/how-hosting-works). | No |
| `INVALID_WEBSITE_STATUS` | — | Website status value isn't valid | Check the status you're setting. | No |
| `INVALID_TARGET_TYPE` | — | Target type isn't valid | Must be `ipfs` or `ipns`. | No |
| `INVALID_TIMESTAMP` | — | Timestamp format is invalid | Use ISO 8601 format. | No |

## SSL

| Code | HTTP | When it happens | What to do | Retry? |
|------|------|----------------|------------|--------|
| `INVALID_SSL_STATUS` | — | SSL status value isn't valid | Check the status you're setting. | No |
| `SSL_STATUS_UPDATE_FAILED` | — | Couldn't update SSL status | Check domain ownership. Retry with backoff. | Yes |
...
| `INVALID_ZONE_STATUS` | — | DNS zone status is invalid | Check the status value. | No |
| `ZONE_NOT_FOUND` | 404 | DNS zone doesn't exist | Create the zone first. See [DNS and Domains](/websites/concepts/dns-and-domains). | No |
| `RECORD_NOT_FOUND` | 404 | DNS record doesn't exist | Check the record ID. | No |
| `INVALID_RECORD_TYPE` | — | DNS record type isn't supported | Use a supported record type (A, AAAA, CNAME, TXT, MX, NS). | No |
