...
:::tip
Using an onchain name like `.eth`? The `pinner point` command handles IPNS key creation, publishing, and updates in one step. See [Onchain Names Quickstart](/onchain/quickstart).
:::
...
```typescript [SDK]
await pinner.ipns.republish(key.ipns_name);
```
:::
