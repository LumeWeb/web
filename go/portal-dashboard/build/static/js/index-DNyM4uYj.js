...
import { federationRuntime } from './virtual_mf-REMOTE_ENTRY_ID-DvKLxUdJ.js';
...
import { dashboard__loadShare__react_mf_2_router__loadShare__, dashboard__loadShare___mf_0_lumeweb_mf_1_portal_mf_2_framework_mf_2_ui_mf_2_core__loadShare__ } from './dashboard__loadShare__react_mf_2_router__loadShare__-GzYn9uIj.js';
...
function getApiBaseUrl(options = {}) {
  const { allowLocalhost = env.VITE_PORTAL_ALLOW_LOCALHOST, currentUrl = getCurrentLocation().href, preserveSubdomain: explicitPreserveSubdomain } = options;
...
var Framework = class {
	...
	async initialize() {
		...
		for (const pluginId of this.#plugins.getEnabledPlugins()) {
			...
			else plugin.capabilities?.forEach((capability) => {
		...
		for (const plugin of this.getPlugins()) if (plugin.features) for (const feature of plugin.features) try {
...
//#region src/components/WidgetArea.tsx
...
export { Builder, ErrorDisplay, Framework, FrameworkProvider, HostContextBridge, MockFrameworkProvider, PluginManager, RemoteContextConsumer, RouteErrorBoundary, RouteErrorBoundaryFallback, RouteLoading, WidgetArea, __test_clearCache, cleanProtocolString, cleanTrailingSlashes, createBridgeComponent, createNamespacedId, createRemoteComponentLoader, defaultRemoteOptions, ensureResource, env, fetchPortalMeta, getAccountSubdomain, getApiBaseUrl, getCurrentLocation, getDefaultRefineOptions, getPluginMeta, getPortalPluginManifests, getProtocolDomain, isNamespacedId, mergeRefineConfig, normalizeDataProvider, normalizeId, parseNamespacedId, registerBridgedContext, resetCurrentLocation, setCurrentLocation, useFramework, useFrameworkLoading, validateNamespacedId };
