...
import { i as global, B as Buffer2, j as Buffer$1, p as path } from "./index-Cb6cVh9L.js";
import { k as getDefaultExportFromCjs$1, n as commonjsGlobal } from "./index-BIsqO_uY.js";
...
function querystring(query) {
  ...
  while (part = parser.exec(query)) {
    var key = decode$2(part[1]), value = decode$2(part[2]);
...
function optionsToConfig(client, def, ...options) {
  ...
  const headers = new AxiosHeaders2(config.headers);
...
class Multihash {
  ...
  static fromBase64Url(hash) {
    ...
    const bytes2 = base64url.decode(hash);
...
class DirectoryReference {
  ...
  toJson() {
    return {
      ...
      encryptionKey: this.encryptionKey ? base64url.encode(this.encryptionKey) : null,
      ext: this.ext
    };
  }
  static decode(data) {
...
const encode = (src2, urlsafe = false) => urlsafe ? _mkUriSafe(_encode(src2)) : _encode(src2);
const encodeURI = (src2) => encode(src2, true);
