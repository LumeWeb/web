import { jsxRuntimeExports, core_dashboard__loadShare___mf_0_lumeweb_mf_1_portal_mf_2_framework_mf_2_ui__loadShare__, core_dashboard__loadShare__react__loadShare__ } from './core_dashboard__loadShare___mf_0_lumeweb_mf_1_portal_mf_2_framework_mf_2_ui__loadShare__-D-EDec9Y.js';
...
import { Dropzone, FileText, Check, Folder, filesize, UploadProgress, Upload } from './Dropzone-BquNXoac.js';
import { createLucideIcon } from './createLucideIcon-a23Vw1TY.js';
import { object, string, array, _instanceof } from './schemas-BzkPIUef.js';
...
const storageSchema = object({
  storage: string().min(1)
});
const noServicesStorageSchema = object({
  storage: string().optional()
});
const filesSchema = object({
  files: array(_instanceof(File)).min(1, "Please select at least one file")
});
object({
  files: array(_instanceof(File)).optional(),
  storage: string().optional()
});
const completeSchema = object({
  fileIds: array(string()).optional(),
  files: array(_instanceof(File)).min(1),
  storage: string().min(1)
});
