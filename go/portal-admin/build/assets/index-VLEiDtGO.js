...
import { j as createLucideIcon, m as anyType, Z as ZodArray, n as ZodObject, o as ZodEffects, p as ZodOptional, q as ZodDefault, r as ZodCatch, s as ZodIntersection, v as ZodUnion, w as ZodDiscriminatedUnion, x as ZodTuple, y as ZodNullable, A as ZodPipeline, D as lazyType, z, E as usePluginMeta, B as Button } from "./createLucideIcon-BCL4v026.js";
import { F as Field, d as FieldCheckbox, S as Sheet, e as SheetTrigger, f as SheetContent, g as SheetHeader, h as SheetTitle, I as Input, u as useFeatureFlag, L as LumeLogo, i as discordLogoPng, l as lumeColorLogoPng } from "./useFeatureFlag-DaT8CAZ9.js";
...
import { S as Search } from "./search--AbUQISB.js";
/**
...
function formatName$1(prefix, path) {
  return typeof path !== "undefined" ? formatPaths$1([...getPaths$1(prefix), path]) : prefix !== null && prefix !== void 0 ? prefix : "";
}
function isPrefix(name, prefix) {
  var paths = getPaths$1(name);
  var prefixPaths = getPaths$1(prefix);
  return paths.length >= prefixPaths.length && prefixPaths.every((path, index) => paths[index] === path);
}
function setValue$1(target, name, valueFn) {
  var paths = getPaths$1(name);
...
function getValue$1(target, name) {
  var pointer = target;
  for (var path of getPaths$1(name)) {
    ...
    if (isPlainObject$1(pointer) && typeof path === "string") {
...
function normalize$1(value) {
  ...
  if (isPlainObject$1(value)) {
    var obj = Object.keys(value).sort().reduce((result, key) => {
      var data = normalize$1(value[key], acceptFile);
  ...
  if (Array.isArray(value)) {
    ...
    return value.map((item) => normalize$1(item, acceptFile));
  }
  if (typeof value === "string" && value === "" || value === null || isFile$1(value) && (!acceptFile || value.size === 0)) {
    return;
  }
  return value;
}
function flatten(data) {
  ...
  function process(data2, prefix) {
    var value = normalize$1(resolve(data2));
...
function invariant$1(expectedCondition, message) {
...
function requestSubmit(form, submitter) {
  invariant$1(!!form, "Failed to submit the form. The element provided is null or undefined.");
...
var INTENT$1 = "__intent__";
var STATE$1 = "__state__";
function getSubmissionContext$1(body) {
  var intent = body.get(INTENT$1);
  var state = body.get(STATE$1);
  invariant$1((typeof intent === "string" || intent === null) && (typeof state === "string" || state === null), 'The input name "'.concat(INTENT$1, '" and "').concat(STATE$1, '" are reserved by Conform. Please use another name for your input.'));
  var context = {
    payload: {},
    fields: /* @__PURE__ */ new Set(),
    intent: getIntent$1(intent)
  };
  if (state) {
    context.state = JSON.parse(state);
  }
  var _loop = function _loop2(next2) {
    if (name === INTENT$1 || name === STATE$1) {
      return 1;
    }
    context.fields.add(name);
    setValue$1(context.payload, name, (prev) => {
      if (!prev) {
        return next2;
...
function getIntent$1(serializedIntent) {
  if (!serializedIntent) {
    return null;
  }
  ...
  switch (intent.type) {
    case "insert":
      return JSON.stringify({
        type: intent.type,
        payload: _objectSpread2$3(_objectSpread2$3({}, intent.payload), {}, {
...
function updateList$1(list, intent) {
  var _intent$payload$index;
  invariant$1(Array.isArray(list), "Failed to update list. The value is not an array.");
  switch (intent.type) {
    case "insert":
...
function setListValue$1(data, intent) {
  setValue$1(data, intent.payload.name, (value) => {
    var list = value !== null && value !== void 0 ? value : [];
    updateList$1(list, intent);
    return list;
  });
}
...
function setState(state, name, valueFn) {
  ...
  var _loop2 = function _loop22() {
    ...
    if (isPrefix(key, name) && key !== name) {
      setValue$1(target, key, (currentValue) => {
  ...
  var result = valueFn(getValue$1(target, name));
  Object.assign(state, flatten(result, {
    resolve(data) {
      if (isPlainObject$1(data) || Array.isArray(data)) {
        var _data$root;
...
function serialize(defaultValue) {
  if (isPlainObject$1(defaultValue)) {
...
function createFormMeta(options, initialized) {
  ...
  var result = {
    ...
    key: !initialized ? getDefaultKey(defaultValue) : _objectSpread2$3({
      "": generateId()
  ...
  return Object.entries(flatten(defaultValue, {
    ...
    if (Array.isArray(value)) {
      for (var i = 0; i < value.length; i++) {
        result[formatName$1(key, i)] = generateId();
      }
    }
    return result;
  }, {});
}
function setFieldsValidated(meta2, fields) {
  for (var _name of Object.keys(meta2.error).concat(fields !== null && fields !== void 0 ? fields : [])) {
    meta2.validated[_name] = true;
  }
}
function handleIntent(meta2, intent, fields, initialized) {
  var _fields$filter;
  if (!intent) {
    setFieldsValidated(meta2, fields);
    return;
  }
  switch (intent.type) {
    case "validate": {
      if (intent.payload.name) {
        meta2.validated[intent.payload.name] = true;
      } else {
        setFieldsValidated(meta2, fields);
      }
      break;
    ...
    case "update": {
      ...
      var _name2 = formatName$1(intent.payload.name, intent.payload.index);
      if (typeof value !== "undefined") {
        updateValue(meta2, _name2 !== null && _name2 !== void 0 ? _name2 : "", value);
      }
      if (typeof validated !== "undefined") {
        if (_name2) {
          setState(meta2.validated, _name2, () => void 0);
        } else {
          meta2.validated = {};
        }
        if (validated) {
          if (isPlainObject$1(value) || Array.isArray(value)) {
            Object.assign(meta2.validated, flatten(value, {
              resolve() {
                return true;
              },
              prefix: _name2
          ...
          meta2.validated[_name2 !== null && _name2 !== void 0 ? _name2 : ""] = true;
        } else if (_name2) {
          delete meta2.validated[_name2];
        }
      }
      break;
    }
    case "reset": {
      var _name3 = formatName$1(intent.payload.name, intent.payload.index);
      var _value = getValue$1(meta2.defaultValue, _name3);
      updateValue(meta2, _name3, _value);
      if (_name3) {
        setState(meta2.validated, _name3, () => void 0);
        delete meta2.validated[_name3];
      } else {
        meta2.validated = {};
      }
      break;
    ...
    case "reorder": {
      if (initialized) {
        meta2.initialValue = clone(meta2.initialValue);
        meta2.key = clone(meta2.key);
        setListState(meta2.key, intent, (defaultValue) => {
          if (!Array.isArray(defaultValue) && !isPlainObject$1(defaultValue)) {
            return generateId();
          }
        ...
        setListValue$1(meta2.initialValue, intent);
      }
      setListState(meta2.validated, intent);
      meta2.validated[intent.payload.name] = true;
      break;
    }
  }
  var validatedFields = (_fields$filter = fields === null || fields === void 0 ? void 0 : fields.filter((name) => meta2.validated[name])) !== null && _fields$filter !== void 0 ? _fields$filter : [];
  meta2.error = Object.entries(meta2.error).reduce((result, _ref3) => {
    var [name, error] = _ref3;
    if (meta2.validated[name] || validatedFields.some((field) => isPrefix(name, field))) {
      result[name] = error;
    }
    return result;
  }, {});
}
function updateValue(meta2, name, value) {
  if (name === "") {
    meta2.initialValue = value;
    meta2.value = value;
    meta2.key = _objectSpread2$3(_objectSpread2$3({}, getDefaultKey(value)), {}, {
      "": generateId()
    });
    return;
  }
  meta2.initialValue = clone(meta2.initialValue);
  meta2.value = clone(meta2.value);
  meta2.key = clone(meta2.key);
  setValue$1(meta2.initialValue, name, () => value);
  setValue$1(meta2.value, name, () => value);
  if (isPlainObject$1(value) || Array.isArray(value)) {
    setState(meta2.key, name, () => void 0);
    Object.assign(meta2.key, getDefaultKey(value, name));
  }
  meta2.key[name] = generateId();
}
...
function createValueProxy(value) {
  var val = normalize$1(value);
  return createStateProxy((name, proxy) => {
    if (name === "") {
      return val;
    }
    var paths = getPaths$1(name);
    var basename = formatPaths$1(paths.slice(0, -1));
    var key = formatPaths$1(paths.slice(-1));
    var parentValue = proxy[basename];
    return getValue$1(parentValue, key);
  });
}
function createConstraintProxy(constraint) {
  return createStateProxy((name, proxy) => {
    ...
    if (!result) {
      ...
      var alternative = formatPaths$1(paths);
      if (name !== alternative) {
...
function createKeyProxy(key) {
  return createStateProxy((name, proxy) => {
    ...
    var parentKey = proxy[formatPaths$1(paths.slice(0, -1))];
...
function createFormContext$1(options) {
  ...
  var meta2 = createFormMeta(options);
  var state = createFormState(meta2);
  ...
  function getSerializedState() {
    return JSON.stringify({
      validated: meta2.validated
    });
  }
  function submit(event) {
    var form = event.target;
    var submitter = event.submitter;
    invariant$1(form === getFormElement(), "The submit event is dispatched by form#".concat(form.id, " instead of form#").concat(latestOptions.formId));
    ...
    if (submission.status === "success" || submission.error !== null) {
      ...
      report(_objectSpread2$3(_objectSpread2$3({}, _result2), {}, {
        status: _result2.status !== "success" ? _result2.status : void 0
      }));
    }
    return _objectSpread2$3(_objectSpread2$3({}, result), {}, {
      submission
    });
  }
  ...
  function willValidate(element, eventName) {
    ...
    var validated = meta2.validated[element.name];
    return validated ? shouldRevalidate === eventName && (eventName === "onInput" || meta2.isValueUpdated) : shouldValidate === eventName;
  }
  function updateFormValue(form) {
    var formData = new FormData(form);
    var result = getSubmissionContext$1(formData);
    updateFormMeta(_objectSpread2$3(_objectSpread2$3({}, meta2), {}, {
      isValueUpdated: true,
  ...
  function report(result) {
    ...
    var error = Object.entries((_result$error = result.error) !== null && _result$error !== void 0 ? _result$error : {}).reduce((result2, _ref5) => {
      ...
      var error2 = newError === null ? meta2.error[name] : newError;
      if (error2) {
        result2[name] = error2;
      }
      return result2;
    }, {});
    var update = _objectSpread2$3(_objectSpread2$3({}, meta2), {}, {
      isValueUpdated: false,
      submissionStatus: result.status,
      value: result.initialValue,
      validated: _objectSpread2$3(_objectSpread2$3({}, meta2.validated), (_result$state = result.state) === null || _result$state === void 0 ? void 0 : _result$state.validated),
      error
    });
    ...
    if (formElement && result.status === "error") {
      for (var element of formElement.elements) {
        if (isFieldElement(element) && meta2.error[element.name]) {
          element.focus();
          break;
        }
      }
...
function updateSubjectRef(ref, subject, scope, name) {
  if (subject === "status" || subject === "formId") {
    ...
    ref.current[subject] = _objectSpread2$2(_objectSpread2$2({}, ref.current[subject]), {}, {
...
function getFieldMetadata(context, subjectRef, stateSnapshot) {
  ...
  var name = typeof key === "undefined" ? prefix : formatPaths$1([...getPaths$1(prefix), key]);
  return new Proxy({}, {
    get(_, key2, receiver) {
  ...
  return _objectSpread2$2(_objectSpread2$2({}, context), {}, {
    submit(event) {
      ...
      if (!result.submission || result.submission.status === "success" || result.submission.error === null) {
        if (!result.formData.has(INTENT$1)) {
          var _onSubmit;
...
function useForm(options) {
  var {
    id
  } = options, formConfig = _objectWithoutProperties$1(options, _excluded$1);
  var formId = useFormId(id);
  var [context] = reactExports.useState(() => createFormContext(_objectSpread2$2(_objectSpread2$2({}, formConfig), {}, {
    formId
  })));
  useSafeLayoutEffect(() => {
  ...
  useSafeLayoutEffect(() => {
    context.onUpdate(_objectSpread2$2(_objectSpread2$2({}, formConfig), {}, {
      formId
    }));
  });
...
function getFormProps(metadata, options) {
  return simplify(_objectSpread2$2({
    id: metadata.id,
      ...
      Object.assign(data, def.options.map((option) => {
        ...
        for (var _name of list) {
          ...
          if (prevConstraint && nextConstraint) {
            ...
            result2[_name] = _objectSpread2$1(_objectSpread2$1(_objectSpread2$1({}, prevConstraint), nextConstraint), {}, {
              required: false
            });
          }
        }
...
function replySubmission(context) {
  ...
  var error = context.error || extraError ? _objectSpread2(_objectSpread2({}, context.error), extraError) : void 0;
...
function enableTypeCoercion(type) {
  ...
  if (def.typeName === "ZodString" || def.typeName === "ZodLiteral" || def.typeName === "ZodEnum" || def.typeName === "ZodNativeEnum") {
    ...
    schema = anyType().transform((value) => {
      ...
      return [value];
    }).pipe(new ZodArray(_objectSpread2$1(_objectSpread2$1({}, def), {}, {
    ...
    schema = anyType().transform((value) => {
      ...
      return value;
    }).pipe(new ZodObject(_objectSpread2$1(_objectSpread2$1({}, def), {}, {
    ...
    if (isFileSchema(type)) {
      ...
      schema = new ZodEffects(_objectSpread2$1(_objectSpread2$1({}, def), {}, {
        schema: enableTypeCoercion(def.schema, cache)
      }));
    }
  } else if (def.typeName === "ZodOptional") {
    schema = anyType().transform((value) => coerceFile(coerceString(value))).pipe(new ZodOptional(_objectSpread2$1(_objectSpread2$1({}, def), {}, {
      innerType: enableTypeCoercion(def.innerType, cache)
    })));
  } else if (def.typeName === "ZodDefault") {
    schema = anyType().transform((value) => coerceFile(coerceString(value))).pipe(new ZodDefault(_objectSpread2$1(_objectSpread2$1({}, def), {}, {
      defaultValue: () => {
    ...
    schema = new ZodCatch(_objectSpread2$1(_objectSpread2$1({}, def), {}, {
      innerType: enableTypeCoercion(def.innerType, cache)
    }));
  } else if (def.typeName === "ZodIntersection") {
    schema = new ZodIntersection(_objectSpread2$1(_objectSpread2$1({}, def), {}, {
      left: enableTypeCoercion(def.left, cache),
      right: enableTypeCoercion(def.right, cache)
    }));
  } else if (def.typeName === "ZodUnion") {
    schema = new ZodUnion(_objectSpread2$1(_objectSpread2$1({}, def), {}, {
      options: def.options.map((option) => enableTypeCoercion(option, cache))
    }));
  } else if (def.typeName === "ZodDiscriminatedUnion") {
    schema = new ZodDiscriminatedUnion(_objectSpread2$1(_objectSpread2$1({}, def), {}, {
      options: def.options.map((option) => enableTypeCoercion(option, cache)),
    ...
    schema = new ZodTuple(_objectSpread2$1(_objectSpread2$1({}, def), {}, {
      items: def.items.map((item) => enableTypeCoercion(item, cache))
    }));
  } else if (def.typeName === "ZodNullable") {
    schema = new ZodNullable(_objectSpread2$1(_objectSpread2$1({}, def), {}, {
      innerType: enableTypeCoercion(def.innerType, cache)
    }));
  } else if (def.typeName === "ZodPipeline") {
    schema = new ZodPipeline(_objectSpread2$1(_objectSpread2$1({}, def), {}, {
...
const LoginForm = () => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("form", { className: "w-full max-w-md ", ...getFormProps(form), children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { className: "w-full h-14", children: "Login" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "inline-block mt-4 text-input-placeholder", children: [
...
function SiOkta(props) {
  return GenIcon({ "tag": "svg", "attr": { "role": "img", "viewBox": "0 0 24 24" }, "child": [{ "tag": "path", "attr": { "d": "M12 0C5.389 0 0 5.35 0 12s5.35 12 12 12 12-5.35 12-12S18.611 0 12 0zm0 18c-3.325 0-6-2.675-6-6s2.675-6 6-6 6 2.675 6 6-2.675 6-6 6z" }, "child": [] }] })(props);
}
...
const socialLoginProviders = /* @__PURE__ */ new Map([
...
function SocialLogin() {
  const socialLoginProvidersList = usePluginMeta("dashboard", "social_providers") || Array.from(socialLoginProviders.keys());
  const accountSubdomain = useAccountSubdomain();
  const parsed = Te();
  const [searchTerm, setSearchTerm] = reactExports.useState("");
  const [isExtraOpen, setIsExtraOpen] = reactExports.useState(false);
  const orderedProviders = socialLoginProvidersList.filter((provider) => socialLoginProviders.has(provider)).map((provider) => ({
    key: provider,
  ...
  const filteredOptions = remainingProviders.filter(
  ...
  const handleLogin = (providerId) => {
    ...
    let toPath = ((_a = parsed.params) == null ? void 0 : _a.to) || "/";
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-full max-w-[300px] space-y-3 mr-10", children: [
    visibleProviders.map(({ key, name, icon: Icon, bgColor }) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
      Button,
      {
        variant: "outline",
    ...
    remainingProviders.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(Sheet, { open: isExtraOpen, onOpenChange: setIsExtraOpen, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(SheetTrigger, { asChild: true, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
        Button,
        {
          ...
          children: [
            "More options",
            /* @__PURE__ */ jsxRuntimeExports.jsx(Ellipsis, { className: "w-5 h-5 m-2" })
          ]
        }
      ) }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(SheetContent, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(SheetHeader, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(SheetTitle, { children: "More login options" }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "py-4", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Search, { className: "absolute left-2 top-3 h-4 w-4 text-gray-500" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            Input,
            {
              type: "search",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "space-y-2", children: filteredOptions.map(({ key, name, icon: Icon, bgColor }) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
          Button,
          {
            variant: "outline",
...
const LoginComponent = ({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "h-screen relative sm:overflow-hidden", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col sm:flex-row-reverse items-center justify-center w-full h-full", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("header", { className: "absolute z-50 top-4 left-4 sm:left-8", children: /* @__PURE__ */ jsxRuntimeExports.jsx(LumeLogo, {}) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative w-full h-full ", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "absolute inset-0 flex sm:hidden flex-col items-start justify-center gap-2 text-left p-4 mt-60 sm:mt-10 ", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-4xl sm:text-3xl font-bold", children: "Welcome back" }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-col items-start justify-start bg-background w-full sm:max-w-md ", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "sm:mt-20 p-4 sm:p-10", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "hidden sm:flex flex-col items-start justify-center gap-2 text-left mb-10", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-4xl mb-2", children: "Welcome back" }),
        showRegister && /* @__PURE__ */ jsxRuntimeExports.jsx(
          InlineAuthLinkBanner,
          {
            to: registerUrl,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("footer", { className: "my-5", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("ul", { className: "flex flex-row sm:flex-row gap-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: /* @__PURE__ */ jsxRuntimeExports.jsx(Link$1, { to: "https://discord.lumeweb.com", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
          Button,
          {
            variant: "link",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: /* @__PURE__ */ jsxRuntimeExports.jsx(Link$1, { to: "https://lumeweb.com", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
          Button,
          {
            variant: "link",
            className: "flex flex-row gap-x-2 text-input-placeholder",
...
function Login() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(LoginComponent, {
    showRegister: false
  });
}
export {
