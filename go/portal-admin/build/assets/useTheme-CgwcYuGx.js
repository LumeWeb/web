...
const postApiAccountVerifyEmailResend = (resendVerifyEmailRequest, options) => {
  return customInstance(
    {
...
const deleteApiAccountDelete = (options) => {
  return customInstance(
    {
...
const postApiAuthOtpVerify = (oTPVerifyRequest, options) => {
  return customInstance(
    {
...
const postApiAuthOtpValidate = (oTPValidateRequest, options) => {
  return customInstance(
    {
      url: `/api/auth/otp/validate`,
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
class AccountApi {
  ...
  async validateOtp(otpValidateRequest) {
    ...
    const success = this.checkSuccessVal(ret);
    if (success) {
      this._jwtToken = ret.token;
      return success;
    }
...
function useSdk() {
  ...
  const initializationAttempted = reactExports.useRef(false);
  reactExports.useEffect(() => {
    if (portalUrl && !isGloballyInitialized && !initializationAttempted.current) {
